...
from ai_trading_agent.trading_engine.models import Order, Fill
...
class SimulatedExecutionHandler(ExecutionHandlerABC, BaseExecutionHandler):
    ...
    """
    def __init__(self, portfolio_manager: PortfolioManager, config: Optional[Dict[str, Any]] = None):
        """
    ...
    def execute_order(self, order: Order, market_data: Dict[str, pd.Series]) -> Optional[Order]:
        ...
        if order.order_type == 'MARKET':
            ...
            if execution_price is None:
                logging.debug(f"Limit order {order.order_id} ({symbol} @ {limit_price}) not filled in current bar.")
        ...
        if execution_price is None or pd.isna(execution_price):
            logging.warning(f"Could not determine execution price for order {order.order_id} ({symbol}). Order remains pending.")
        ...
        try:
            ...
            logging.info(f"Order Executed & Filled: {order.order_id} - {order.quantity} {symbol} @ {order.executed_price:.4f} (Commission: {order.commission:.2f})")
        except Exception as e:
            logging.error(f"Error updating portfolio after fill for order {order.order_id}: {e}", exc_info=True)
            # What to do here? Revert order status? Log critical error?
            # For simulation, maybe log and continue, but flag the issue.
            order.status = 'ERROR' # Mark as error state
