...
try:
    ...
    from .portfolio import BasePortfolioManager
...
try:
    ...
    from .portfolio import SimplePortfolioManager # Example concrete class
    ...
    def run(self) -> Optional[Dict[str, Any]]:
        ...
        while self._running and current_time:
            ...
            try:
                ...
                portfolio_state = self.portfolio_manager.get_state() # Get cash, positions, value
            ...
            try:
                strategy_signals = self.strategy_manager.generate_signals(market_data)
                if strategy_signals:
            ...
            # --- Process Executions for this Timestamp --- #
            try:
                executed_trades = self.execution_handler.process_executions()
                if executed_trades:
                    logging.info(f"Executed trades for {current_time}: {len(executed_trades)}")
                    self.results['trades'].extend(executed_trades)
                    # Update portfolio based on executed trades
                    self.portfolio_manager.update_from_executed_trades(executed_trades)
            except Exception as e:
                 logging.error(f"Error processing executions: {e}", exc_info=True)
                 self._running = False # Stop if execution processing fails
                 break
        ...
        if self.portfolio_manager:
             ...
             self.results['portfolio_history'].append({
                 'timestamp': self.data_manager.timestamp_index[-1].to_pydatetime() if self.data_manager.timestamp_index is not None and not self.data_manager.timestamp_index.empty else None,
                 'value': final_state.get('total_value'),
                 'cash': final_state.get('cash'),
                 'positions': final_state.get('positions', {}).copy()
             })
        ...
        if self.results['portfolio_history']:
            ...
            if not history_df.empty:
                try:
                    self.results['performance_metrics'] = calculate_performance_metrics(history_df[['value']])
