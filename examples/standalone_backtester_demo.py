...
class StrategyBacktester:
    ...
    def plot_optimization_results(self, param_names: List[str], top_n: int = 10):
...
def generate_sample_data(symbol: str, timeframe: str, days: int = 365) -> pd.DataFrame:
    """Generate synthetic market data for demonstration purposes with more pronounced trends."""
    logger.info(f"Generating sample data for {symbol} with {timeframe} timeframe")
    
    # Parameters for the random walk
    mu = 0.0001  # Drift
    sigma = 0.015  # Increased volatility for more trading opportunities
    ...
    # Generate base price data using geometric random walk
    returns = np.random.normal(mu, sigma, n_periods)
    price_path = 100 * np.exp(np.cumsum(returns))
    
    # Add more pronounced trends and cycles to create trading opportunities
    
    # Long-term trend
    long_trend = np.linspace(0, 30, n_periods)
    
    # Medium cycles (roughly 20-30 day cycles)
    medium_cycles = 15 * np.sin(np.linspace(0, 12 * np.pi, n_periods))
    
    # Short cycles (roughly 5-10 day cycles)
    short_cycles = 7 * np.sin(np.linspace(0, 30 * np.pi, n_periods))
    
    # Combine components with the random walk
    price_path = price_path + medium_cycles + short_cycles
    
    # Add a few trend changes to create crossover opportunities
    for i in range(5):
        # Random position for trend change
        pos = np.random.randint(n_periods // 10, n_periods - n_periods // 10)
        # Length of the trend change
        length = np.random.randint(n_periods // 30, n_periods // 15)
        # Magnitude of the trend change (up or down)
        magnitude = np.random.choice([-1, 1]) * np.random.uniform(5, 15)
        
        # Apply a linear trend change
        trend_change = np.zeros(n_periods)
        for j in range(length):
            if pos + j < n_periods:
                trend_change[pos + j] = magnitude * (j / length)
        
        price_path = price_path + trend_change
...
def main():
    ...
    logger.info("-" * 50)
    logger.info("Technical Backtester Demo Completed (Standalone Version)")
