"""
Standalone example demonstrating an alert system for a trading platform.

This script implements a simple alert system with various levels, categories, and handlers.
It doesn't depend on any other modules in the project.
"""
...
async def generate_exchange_alerts(alert_manager: AlertManager):
...
async def generate_order_alerts(alert_manager: AlertManager):
...
async def generate_risk_alerts(alert_manager: AlertManager):
...
async def display_alerts(alert_manager: AlertManager):
...
async def main():
    ...
    await generate_exchange_alerts(alert_manager)
    await asyncio.sleep(1)
    
    await generate_order_alerts(alert_manager)
    await asyncio.sleep(1)
    
    await generate_risk_alerts(alert_manager)
    await asyncio.sleep(1)
    
    # Display all active alerts
    await display_alerts(alert_manager)
