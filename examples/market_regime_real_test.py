...
class MarketRegimeValidator:
    ...
    def classify_regime(self, data):
        ...
        latest_idx = -1
        volatility = data_copy['volatility'].iloc[latest_idx]
        price = data_copy['Close'].iloc[latest_idx]
        ma50 = data_copy['ma50'].iloc[latest_idx]
        ma200 = data_copy['ma200'].iloc[latest_idx]
...
def test_market_periods(data, symbol):
    ...
    periods = [
        {"name": "Bull Market", "start": pd.Timestamp("2019-01-01"), "end": pd.Timestamp("2019-12-31")},
        {"name": "COVID Crash", "start": pd.Timestamp("2020-02-15"), "end": pd.Timestamp("2020-04-15")},
        {"name": "Recovery", "start": pd.Timestamp("2020-04-16"), "end": pd.Timestamp("2020-12-31")},
        {"name": "2021 Bull Run", "start": pd.Timestamp("2021-01-01"), "end": pd.Timestamp("2021-11-30")},
        {"name": "2022 Bear Market", "start": pd.Timestamp("2022-01-01"), "end": pd.Timestamp("2022-10-31")}
    ]
    ...
    for i, period in enumerate(periods):
        ...
        period_data = data[symbol].loc[
            (data[symbol].index >= period['start']) & 
            (data[symbol].index <= period['end'])
        ]
