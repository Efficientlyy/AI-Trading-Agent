#!/usr/bin/env python
"""
Final End-to-End Pipeline Test with Format-Compatible Data Service
...
Uses format-compatible data service that properly handles MEXC API responses.
"""
...
try:
    # Use format-compatible version of MultiAssetDataService
    from format_compatible_data_service import MultiAssetDataService
...
def run_end_to_end_test():
    ...
    # If no data was collected, try direct API calls
    if not has_data:
        logger.warning("No data collected through pipeline, trying direct API calls")
        try:
            for symbol in config['symbols']:
                for timeframe in config['timeframes']:
                    candles = data_service.get_klines(symbol, timeframe)
                    if candles and len(candles) > 0:
                        has_data = True
                        market_data[symbol][timeframe] = candles
                        logger.info(f"Directly collected {len(candles)} candles for {symbol} {timeframe}")
        except Exception as e:
            logger.error(f"Error in direct API calls: {str(e)}")
    ...
    if not has_data:
        ...
        save_results({
            ...
            'signals': [],
            'trades': [],
            'errors': ["Market data collection failed for all timeframes"]
        })
    ...
    for symbol in config['symbols']:
        try:
            ...
            available_timeframes = [tf for tf in config['timeframes'] if len(market_data[symbol].get(tf, [])) > 0]
            if not available_timeframes:
                logger.warning(f"No data available for any timeframe for {symbol}")
                continue
                
            timeframe = '15m' if '15m' in available_timeframes else available_timeframes[0]
            ...
            if len(candles) > 0:
                ...
                # Test orderbook directly to verify it's working
                orderbook = data_service.get_order_book(symbol)
                logger.info(f"Orderbook test for {symbol}: {orderbook}")
                
                # Generate signal
                signal = signal_generator.generate_signals(symbol)
                logger.info(f"Signal generation result for {symbol}: {signal}")
