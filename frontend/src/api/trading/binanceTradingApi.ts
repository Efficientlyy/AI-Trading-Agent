...
const createBinanceClient = (tradingMode: TradingMode, config: BinanceConfig): AxiosInstance => {
  const baseURL = tradingMode === 'live' ? BINANCE_API_URL : BINANCE_TEST_API_URL;
...
// Execute Binance API call with circuit breaker pattern
...
export const binanceTradingApi = (tradingMode: TradingMode, config: BinanceConfig): TradingApi => {
  ...
  return {
    ...
    async getPortfolio(): Promise<Portfolio> {
      return executeBinanceCall<Portfolio>(
        'getPortfolio',
        async () => {
    ...
    async getBalance(asset?: string): Promise<number> {
      return executeBinanceCall(
        ...
        async () => {
          // Get account information
          const { data: account } = await client.get('/api/v3/account');
      ...
      return executeBinanceCall(
        ...
        async () => {
          ...
          const allOrders = [...openOrders, ...orderHistory].map((order: any) => ({
            ...
            updatedAt: new Date(order.transactTime),
