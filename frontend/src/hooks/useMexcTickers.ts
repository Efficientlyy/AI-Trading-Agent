...
import mexcService, { MexcTicker } from '../api/mexcService';

// This constant should match the one in useMexcData.ts
const USE_MOCK_DATA = true;

// Export the interface so it can be used in components that use this hook
      ...
      try {
        ...
        if (USE_MOCK_DATA) {
          ...
          const filteredTickers = Array.isArray(response) 
            ? response.filter(ticker => supportedPairs.includes(ticker.symbol))
            : [response].filter(ticker => supportedPairs.includes(ticker.symbol));
          
          // Convert to our format
          const formattedTickers: TickerItem[] = filteredTickers.map(ticker => ({
