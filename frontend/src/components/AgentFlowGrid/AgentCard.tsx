...
import SentimentDetailView from './SentimentDetailView2';
        ...
        <CardContent sx={{ p: 2 }}>
          ...
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
              <Typography variant="body2" sx={{ color: '#9e9e9e', fontSize: '0.7rem' }}>
                Type: <span style={{ color: '#64b5f6' }}>{agent.type || 'Unknown'}</span>
              </Typography>
              <IconButton 
          ...
          {showDetails && (
            <Box sx={{ mt: 1, p: 1, bgcolor: 'rgba(255,255,255,0.05)', borderRadius: 1 }}>
              ...
              {agent.config_details?.description && (
                <Typography variant="caption" sx={{ display: 'block', mb: 0.5, color: '#aaa' }}>
                  <strong>Description:</strong> {agent.config_details.description}
                </Typography>
              )}
              {agent.config_details?.api_key_status && (
                <Typography variant="caption" sx={{ display: 'block', mb: 0.5, color: '#aaa' }}>
                  <strong>API Key Status:</strong> {agent.config_details.api_key_status}
                </Typography>
              )}
              {agent.last_updated && (
                <Typography variant="caption" sx={{ display: 'block', color: '#aaa' }}>
                  <strong>Last Updated:</strong> {new Date(agent.last_updated).toLocaleString()}
                </Typography>
              )}
            </Box>
          )}
