"""
Script to create a fixed version of the indicator_engine.py file with proper indentation.
This will create a clean version of the file that can be used to replace the problematic one.
"""

import os
import shutil

def create_fixed_indicator_engine():
    """Create a fixed version of the indicator_engine.py file."""
    original_file = 'ai_trading_agent/agent/indicator_engine.py'
    fixed_file = 'ai_trading_agent/agent/indicator_engine_fixed.py'
    
    # Create a sample of the fixed file with proper indentation
    fixed_content = '''"""
...
from ai_trading_agent.utils.cache_manager import CacheManager
...
class IndicatorEngine:
    ...
    def __init__(self, config: Dict[str, Any], 
        ...
        self.logger.critical("INDICATOR_ENGINE __init__: Logger obtained.") # NEW LOG
        self.logger.debug(f"IndicatorEngine.__init__: Logger ID is {id(self.logger)}")
    ...
    def _calculate_rsi(self, df: pd.DataFrame, period: int) -> Dict[str, pd.Series]:
        ...
        if rust_available and hasattr(self.rs_features, 'create_rsi_features_rs'):
            try:
                ...
                # Convert None values to NaN
                rsi_values = [float('nan') if val is None else val for val in rsi_result]
...
'''
    ...
    print(f"Created fixed version of indicator_engine.py at {fixed_file}")
    
    # Now replace the original file with the fixed version
    shutil.copy(fixed_file, original_file)
    print(f"Replaced {original_file} with fixed version")
    
    return True

if __name__ == "__main__":
    create_fixed_indicator_engine()
