...
for cryptocurrency assets from MEXC API with improved symbol handling.
"""
...
class MultiAssetDataService:
    ...
    def __init__(self, supported_assets=None, env_path=None):
        ...
        logger.info(f"Loaded API credentials - Key: {self.api_key[:5]}... Secret: {self.api_secret[:5] if self.api_secret else None}...")
    ...
    def get_klines(self, asset=None, interval="1m", limit=100):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Error fetching klines for {target_asset} (API symbol: {symbol}): {response.status_code}, {response.text}")
                return self.cache[target_asset]["klines"].get(interval, [])
        except Exception as e:
            logger.error(f"Exception fetching klines for {target_asset} (API symbol: {symbol}): {str(e)}")
            return self.cache[target_asset]["klines"].get(interval, [])
    ...
    def get_exchange_info(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Error fetching exchange information: {response.status_code}, {response.text}")
                return {}
        except Exception as e:
            logger.error(f"Exception fetching exchange information: {str(e)}")
            return {}
    ...
    def get_account_info(self):
        ...
        # Generate signature
        signature = self._generate_signature_v2(params)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Error fetching account information: {response.status_code}, {response.text}")
                return {}
        except Exception as e:
            logger.error(f"Exception fetching account information: {str(e)}")
            return {}

# Example usage
if __name__ == "__main__":
    # Test with BTCUSDC
    service = MultiAssetDataService(supported_assets=["BTC/USDC"])
    
    # Test ticker
    ticker = service.get_ticker()
    print(f"Ticker: {ticker}")
    
    # Test klines
    klines = service.get_klines(interval="5m", limit=10)
