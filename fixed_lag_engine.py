"""
Fixed version of the simplified lag engine that correctly calls the Rust functions.
This version matches the actual signature of the Rust functions in features.rs.
"""
...
logger = logging.getLogger("FixedLagEngine")
...
class FixedLagEngine:
    """
    A fixed version of the lag engine that correctly calls the Rust functions.
    This version matches the actual signature of the Rust functions in features.rs.
    """
    
    def __init__(self):
        """Initialize the fixed lag engine."""
        self.logger = logger
    ...
    def calculate_lag_features_rust(self, df: pd.DataFrame, lags: List[int], source_column: str = 'close') -> Dict[str, pd.Series]:
        ...
        try:
            ...
            # Print details before calling Rust function
...
if __name__ == "__main__":
    ...
    # Create the engine
    engine = FixedLagEngine()
    
    # Print Rust availability
    print(f"Rust available: {engine.rust_available}")
    print(f"Has Rust lag features: {engine.has_rust_lag_features}")
    if engine.rs_features is not None:
        print(f"Available Rust functions: {[f for f in dir(engine.rs_features) if not f.startswith('_')]}")
    ...
    if engine.rust_available and engine.has_rust_lag_features:
        print("\nAttempting to use Rust implementation...")
